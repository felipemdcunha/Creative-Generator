...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 2.5 Check for API Key if in AI Studio
  ...
  const handleSetUpdate = (updatedSet: CreativeSet) => {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center text-brand">Carregando...</div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      {needsApiKey && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-brand text-white p-4 flex justify-between items-center shadow-lg">
          <div className="flex items-center gap-3">
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
            <p className="font-medium text-sm">Para gerar imagens de alta qualidade, você precisa selecionar uma chave de API paga do Google Cloud.</p>
          </div>
          <button 
            onClick={handleOpenKeySelect}
            className="bg-white text-brand px-4 py-1.5 rounded-lg text-sm font-bold hover:bg-gray-100 transition"
          >
            Selecionar Chave
          </button>
        </div>
           ...
           {view === 'personas' && (
             <>
               <button 
                 ...
                 className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 border border-blue-200 rounded-lg text-sm font-medium text-blue-800 hover:bg-blue-100 transition shadow-sm"
               >
